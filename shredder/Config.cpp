...
static const QHash<Config::ConfigKey, ConfigDirective> configStrings = {
    ...
    // PasswordGenerator
    {Config::PasswordGenerator_ViewPassword, {QS("PasswordGenerator/ViewPassword"), Roaming, true}},
...
static const QHash<QString, Config::ConfigKey> deprecationMap = {
    ...
    {QS("generator/ViewPassword"), Config::PasswordGenerator_ViewPassword},
...
void Config::init(const QString& configFileName, const QString& localConfigFileName)
{
    ...
    if (!QFile::exists(localConfigFileName)) {
        QString oldLocalConfigPath =
            QStandardPaths::writableLocation(QStandardPaths::GenericCacheLocation) + "/shredder";
        QString suffix;
#ifdef QT_DEBUG
        suffix = "_debug";
#endif
        oldLocalConfigPath += QString("/shredder%1.ini").arg(suffix);
...
QPair<QString, QString> Config::defaultConfigFiles()
{
    ...
    if (isPortable()) {
        return {portableConfigDir().append("/shredder.ini"), portableConfigDir().append("/shredder_local.ini")};
    }
    ...
    configPath = QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation) + "/shredder";
    ...
    localConfigPath = xdgStateHome + "/shredder";
#endif
    ...
    configPath += QString("/shredder%1.ini").arg(suffix);
    localConfigPath += QString("/shredder%1.ini").arg(suffix);
